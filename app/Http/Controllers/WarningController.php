...
class WarningController extends Controller
{
    ...
    public function index()
    {
        ...
        $warning = Warning::orderBy('date', 'desc')->paginate(25);
        $types = Type::all();
    ...
    {      
        ...
        $late = WebhrLate::where('validated', 1)->where('id','>=',70000)->get();
        foreach ($absent as $a) {
            if($a->employee){
        ...
        foreach ($late as $a) {
            if($a->employee){
                $this->offense($a->employee->acc_id, $this->hoursToMinutes($a->late), $a->date);
            }
        }
    ...
    public function webhrWarning_test()
    {      
        $absent = WebhrAbsent::where('validated', 1)->where('id','>=',1000)->get();
        $late = WebhrLate::where('validated', 1)->where('id','>=',50000)->get();
        // foreach ($absent as $a) {
        //     if($a->employee){
        //         $this->calAbsent($a->employee->acc_id, $a->date);
        //     }
